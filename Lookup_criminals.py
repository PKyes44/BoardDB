...
conn = my.connect(host = 'localhost', user = 'root',
                    password = '', cursorclass = cursors.DictCursor,
                    autocommit = True)
# 커서 지정
cur = conn.cursor()   
...
while(1):
    ...
    # 조회   
        ...
        # 범죄이력 조회
            ...
            if target == 'all':
                sql = """
                SELECT crimename as '이름', guilt as '범죄명'
                FROM criminal
                ...
                FROM criminal
                WHERE crimename = '{}'
                ORDER BY crimename ASC;""".format(target)
        # 모든 데이터 조회
        else :
            sql = """
            SELECT crimename as '이름', guilt as '범죄명', crimescore as '범죄점수'
            FROM criminal;"""
